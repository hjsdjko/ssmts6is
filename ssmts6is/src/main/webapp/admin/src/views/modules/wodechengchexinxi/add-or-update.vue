<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="车辆号码" prop="chelianghaoma">
          <el-select v-model="ruleForm.chelianghaoma" placeholder="请选择车辆号码">
            <el-option
                v-for="(item,index) in chelianghaomaOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="车辆号码" prop="chelianghaoma">
	      <el-input v-model="ruleForm.chelianghaoma"
                placeholder="车辆号码" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="到达时间" prop="daodashijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.daodashijian" label="到达时间" prop="daodashijian">
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xueshengxingming!=''&&json.xueshengxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zuoweiguanli/zuowei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zuoweiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wodechengchexinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wodechengchexinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
